...
describe('UserApplication', () => {
	...
	context('when dealing with a single persisted user', () => {
		...
		it('gets a user by ID', () => {
			return client.get(`/users/${persistedUser.id}`).send().expect(200, toJSON(persistedUser));
		});
		...
		it('replaces the user by ID', async () => {
			...
			await client.put(`/users/${persistedUser.id}`).send(updatedUser).expect(204);
		...
		it('deletes the user', async () => {
			...
			await expect(userRepo.findById(persistedUser.id)).to.be.rejectedWith(EntityNotFoundError);
		});
	...
	it('queries users with string-based order filter', async () => {
		const userInProgress = await givenUserInstance({ username: 'vjekooo', id });
	...
	it('queries users with exploded string-based order filter', async () => {
		...
		await client
			...
			.expect(200, [toJSON(userCompleted), toJSON(userCompleted2), toJSON(userInProgress)]);
	});
   ...
   */
	...
	async function givenRunningApplicationWithCustomConfiguration() {
		...
		app.bind('datasources.config.geocoder').to(getProxiedGeoCoderConfig(cachingProxy));
